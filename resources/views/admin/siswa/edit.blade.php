    ...
    <form action="{{ isset($siswa) ? route('admin.siswa.update', $siswa->id) : route('admin.siswa.store') }}" method="POST"
        ...
        <div class="mb-3">
            ...
            <select class="form-select" aria-label="Default select example" name="kelas" id="kelas">
                ...
                <option value="XI I IPA" {{ isset($siswa) && $siswa->kelas == 'XI IPA I' ? 'selected' : '' }}>XI IPA I
                </option>
                <option value="XI I IPS" {{ isset($siswa) && $siswa->kelas == 'XI IPS I' ? 'selected' : '' }}>XI IPS I
                </option>
                <option value="XI A PPLG" {{ isset($siswa) && $siswa->kelas == 'XI PPLG A' ? 'selected' : '' }}>XI PPLG A
                </option>
                <option value="XI B APL" {{ isset($siswa) && $siswa->kelas == 'XI APL B' ? 'selected' : '' }}>XI APL B
                </option>
            </select>
        </div>
        <div class="mb-3">
